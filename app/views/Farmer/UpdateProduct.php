...
<body>
    ...
    <div class="container">
        ...
        <div class="product-form-container">
            <form id="add-product-form" action="<?php echo URLROOT; ?>/FarmerController/UpdateProducts/<?php echo $data['id'];?>" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="closing_date">Closing Date & Time</label>
                                            <?php
                        // Convert SQL datetime to HTML datetime-local format
                        $datetime = '';
                        if (!empty($data['closing_date'])) {
                            $datetime = date('Y-m-d\TH:i', strtotime($data['closing_date']));

                            $today = date('Y-m-d');
  $seven_days_later = date('Y-m-d', strtotime('+7 days'));
                        }
                        ?>
                    <input type="datetime-local" id="closing_date" name="closing_date" class="form-control" value="<?php echo $datetime; ?>" min="<?php echo $today; ?>" max="<?php echo $seven_days_later; ?>" required>
                    <span class="form-invalid">
